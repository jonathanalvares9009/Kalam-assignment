...
const ProfileTooltip = ({needsFollowButton, placement, children}: ProfileTooltipProps) => {
  return (
    <Tooltip
        content={
            ...
            <div className="flex text-blue-100 justify-between">
                ...
                {needsFollowButton && <button className="text-blue-100 border-2 rounded-full border-gray-10 py-1 px-6">Follow</button>}
            </div>
            <div className="flex gap-2">
                {SHOTS.map((shot) => (
                <a href={shot.href} key={shot.key} className="flex flex-col gap-1">
                    <Image src={shot.src} width={100} height={100} alt={shot.alt} className="rounded-md" />
                </a>
                ))
                }
            </div>
