...
import {
  useFeedbackModalStore,
  useShowDemoModalStore,
...
const Navbar = () => {
  ...
  const { isSignUpModalOpen, setClose: setIsSignUpModalOpen } =
    useSignUpModalStore();
  const { isShowDemoModalOpen, setClose: setIsShotDetailsModalOpen } =
    useShowDemoModalStore();
  const { isShowDetailsModalOpen, setClose: setIsShareModalOpen } =
    useShowDetailsModalStore();
    ...
    <nav className="flex justify-between relative z-1 py-5 px-4 h-24 selection:bg-yellow-50">
      <div className="flex items-center gap-4">
        ...
        <a href="https://dribbble.com/" className="cursor-pointer">
          <Image
            ...
            height={30}
            alt="Dribble Logo"
            className="block select-none xl:hidden lg:w-28"
          />
        </a>
        <ul className="hidden h-full gap-6 lg:flex">
          {NAV_LINKS.map((link) => (
            <Link
              href={link.href}
              key={link.key}
              className="regular-16 text-blue-100 font-bold flexCenter cursor-pointer pb-1.5 transition-all hover:font-bold"
            >
              {link.label}
            </Link>
          ))}
      ...
      <div className="flex">
        <div className="items-center border border-gray-300 bg-gray-300 rounded-full px-4 hidden h-full gap-6 xl:flex">
          ...
          <input
            type="text"
            className="outline-none bg-gray-300  px-2 w-full"
            placeholder="Search..."
          />
        </div>
        <a href="https://dribbble.com/search" className="flex xl:hidden">
          <Image
            ...
            height={20}
            alt="Search Icon"
            className="block select-none cursor-pointer mx-4"
          />
        </a>
        <a href="https://dribbble.com/session/new">
          <Button
            type="button"
            title="Log in"
            variant="btn_transparent hidden lg:flex"
          />
        </a>
        ...
        <div className="hidden flex-col fixed top-36 right-8 gap-4 justify-center select-none sm:flex">
          <ProfileTooltip needsFollowButton={false} placement="left">
            <Image
              ...
              height={20}
              alt="Profile"
              className="hover:animate-pulse rounded-full cursor-pointer select-none h-10 w-10"
            />
          </ProfileTooltip>
